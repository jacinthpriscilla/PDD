import { AssessmentAnswers, PredictionResult, RiskCategory } from '../shared/src';
...
export function runRandomForestInference(
  ...
  if (answers.brushingFrequency === '0 times per day' || answers.brushingFrequency === 'Once daily') {
    recommendations.push('Improve oral hygiene by brushing at least twice daily with fluoridated toothpaste.');
  }

  if (answers.interdentalCleaning === 'No' || answers.interdentalFrequency === 'Occasionally') {
    recommendations.push('Use interdental cleaning methods like dental floss or interdental brushes daily.');
  }

  if (answers.gumBleeding === 'Yes' || answers.gumSwelling === 'Yes' || answers.looseTeeth !== 'No') {
    recommendations.push('Schedule dental consultation immediately for clinical attachment and pocket depth assessment.');
  }

  if (answers.diabetes === 'Yes' && answers.bloodSugarControl !== 'Controlled') {
    recommendations.push('Maintain diabetes control in coordination with your primary care physician.');
  }

  if (answers.smokingStatus === 'Current smoker') {
    recommendations.push('Consider smoking cessation options to preserve alveolar bone structure.');
  }

  if (answers.cleaningFrequency === 'Never' || answers.lastDentalVisit === 'More than 1 year ago') {
    recommendations.push('Book professional scaling and prophylaxis session every 3-6 months.');
  }

  if (recommendations.length === 0) {
    recommendations.push('Maintain optimal oral hygiene, daily flossing, and regular bi-annual dental checkups.');
  }
