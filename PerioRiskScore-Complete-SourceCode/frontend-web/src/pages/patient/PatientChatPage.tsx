...
export const PatientChatPage: React.FC = () => {
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    const newMsg = { id: Date.now().toString(), sender: 'patient', text: input, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) };
      ...
      <div className="flex-1 flex">
        ...
        <main className="flex-1 p-6 sm:p-8 flex flex-col h-[calc(100vh-4rem)] overflow-hidden">
          ...
          <div className="pb-4 border-b border-slate-800">
            <h1 className="text-xl font-extrabold text-white">Consultation Channel: Dr. Marcus Vance, DDS</h1>
            <p className="text-xs text-teal-400 font-semibold">Specialist Periodontics & Implantology</p>
          </div>
            ...
            {messages.map(m => (
              <div key={m.id} className={`flex ${m.sender === 'patient' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-md p-3.5 rounded-2xl text-xs leading-relaxed ${
                  m.sender === 'patient' ? 'bg-teal-600 text-white rounded-br-none' : 'glass-card border border-slate-800 text-slate-200 rounded-bl-none'
                }`}>
                  <p>{m.text}</p>
